        ...
        <div class="row">
                ...
                <div>
                    ...
                    @foreach($vendorData as $vendata)
                        ...
                        (day) <b>{{date('d')}} </b>of (Month) <b>{{date('F')}}</b>, (Year) <b>{{date('Y')}}</b>
                            ,<p><center>is between:</center></p><p><center><b>Benito Roger L. De Joya </b> (OWNER) of My Seoul Tiangge </center></p>and <p><center>
                           <b>  {{ ucfirst($vendata->venFName). " ". ucfirst($vendata->venMName). " ". ucfirst($vendata->venLName)}} </b>(VENDOR)</center></p> The OWNER and VENDOR hereby agree the rental of Stall No. <b> {{$vendata->stallID}} </b>located on (Floor) <b> {{$stall->floor->floorNo}} </b>,(Building) <b> {{$stall->floor->building->bldgName}}.</b>
                    ...
                    <div style="margin-left:10%;margin-right:10%;">
                        ...
                        @if ($vendata->rentStartDate != "")                           
                            <b> {{$date =explode('-', $vendata->rentStartDate)[2]}} </b>
                              
                           
                            of (Month) <b> {{
                            $dateObj   = DateTime::createFromFormat('!m', explode('-', $vendata->rentStartDate)[1])->format('F')
                           }} </b>,(Year)       <b> {{$date =explode('-', $vendata->rentStartDate)[0]}} </b>.</p>
                        ...
                        <p>4. Rental fees</p>
                        <p style="text-indent:50px;">Vendor shall pay rent (Php {{
                        number_format($stypeCollection->sratePrice, 2, '.', ',')}} per @if($stypeCollection->collection == 1) {{"day"}}
                                @elseif ($stypeCollection->collection == 2)
                                {{"week"}}
                                @else {{"month"}}
                        ...
                        <table>
                          <tr>
                           ...
                           @foreach($utilityNames as $u)
                           <tr>
                                <td>
                                  {{$u->utilName}}
                               </td>
                               @foreach($util as $uu)
                               @if($uu->utilID == $u->utilID)
                                @if($uu->RateType == 1)
                                     <td>Monthly Reading</td>
                                     <td> {{$uu->meterID}}</td>
                                    @else
                                     <td>Php {{ number_format($uu->Rate, 2, '.', ',') }}</td>
                        ...
                        @foreach($fees as $fee)
                         <p style="text-indent:50px;">Any and all additional fees shall be payable under the same terms as set forth for the rental fees herein. A late fee of @if($fee->penType==1) Php {{  number_format($fee->penAmount, 2, '.', ',')}} 
                         @else {{  $fee->penAmount }} %
                         @endif
                         for @if($fee->feeID == null){{"Overdue Payment"}}
                          @else{{"Registration"}}
                          @endif
                          after {{$fee->penDays}} days
                    ...
                    <div style="margin-top:5%;">
                       ...
                       <center>
                            ...
                            <input type="text" value = "{{ ucfirst($vendata->venFName). ' '. ucfirst($vendata->venMName). ' '. ucfirst($vendata->venLName)}}"
