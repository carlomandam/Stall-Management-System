...
class ContractController extends Controller
{
  ...
  public function viewContract($id) {
      $contract = Contract::with('Stall.Submeter','Product','StallRate')->find($id);
      if(count($contract) == 0){
  ...
  public function changeReading(){
    $stall = Stall::find($_POST['stall']);
    return view("transaction.ManageContracts.ChangeReading",compact('stall'));
  }
